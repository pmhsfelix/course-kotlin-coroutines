...
class AlternativeFlowIntroTests {
    @Test
    fun `using sequences`() {
        val seq =
            sequence {
    ...
    fun <T> ourFlow(block: suspend OurFlowCollector<T>.() -> Unit): OurFlow<T> =
        object : OurFlow<T> {
    ...
    fun <T, R> OurFlow<T>.map(mapper: suspend (T) -> R): OurFlow<R> =
        object : OurFlow<R> {
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(AlternativeFlowIntroTests::class.java)
