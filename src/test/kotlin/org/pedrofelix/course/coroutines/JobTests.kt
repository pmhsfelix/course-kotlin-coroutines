...
class JobTests {
    ...
    fun `job tree`() =
        runBlocking {
            ...
            assertTrue(job0.children.contains(job1), "job0 contains job1 as a child")
            assertTrue(job0.children.contains(job2), "job0 contains job2 as a child")

            assertEquals(
                job0,
                firstCoroutineContext?.get(Job),
                "coroutineContext contains the job returned by launch",
            )
            ...
            assertEquals(
                0,
                job0.children.count(),
                "the parent job doesn't have any child job after completion",
            )
        }
    /*
     * Takeaways:
    ...
    @Test
    fun `using the Job from a scope`() =
        runBlocking {
